...
def make_pileup_rgb_minimizers(pid, readname, readqual, readlen, matches, args):
	try:
		...
		minimizers = matches[0][12]
		avg_stretch = 128.0
		seq = [[255.0, np.mean(readqual[i:i+15])*2.0, avg_stretch] if i+15 <= len(readqual) else [255.0, np.mean(readqual[i:len(readqual)])*2.0, avg_stretch] for i in minimizers]
		pileup.append(seq)
		for i in range(maxdepth):
			pileup.append([[0.0,0.0,0.0]])  # fill in placeholder lines
		...
		for s in selections:
			...
			for pix in range(len(seq)):
				if seq[pix][0] == 255.0 and seq[pix][2] != 0.0:
					...
					seq[pix][1] = np.mean(selection[14][matchind:matchind+15])*2.0 if matchind+15 < len(selection[14]) else np.mean(selection[14][matchind:len(selection[14])])*2.0
